...
export default function ServicePageLayout({ locale, serviceData, skipMainLayout = false }: ServicePageLayoutProps) {
  ...
  const renderContent = () => {
    // Safety check - if serviceData is undefined, show error state
    if (!serviceData) {
      return (
        <div className="py-24 text-center">
          <Container>
            <h2 className="text-2xl font-bold mb-4">{isRtl ? 'بيانات الخدمة غير متوفرة' : 'Service data unavailable'}</h2>
            <p className="mb-6">{isRtl ? 'لم يتم العثور على محتوى الخدمة المطلوبة' : 'The requested service content could not be found'}</p>
      ...
      <React.Fragment>
        ...
        <section className="relative bg-gradient-to-r from-primary-900 to-primary-800 text-white py-24 pt-32">
          ...
          <Container>
            ...
            >
                          <h1 className="text-4xl md:text-5xl font-bold mb-4">{serviceData?.title || 'Service'}</h1>
            <p className="text-xl opacity-90 mb-8">
              {serviceData?.subtitle || ''}
            </p>
              <Button
                href={serviceData?.cta?.buttonUrl || `/contact?service=${encodeURIComponent(serviceData?.title || 'Service')}`}
                variant="secondary"
                size="lg"
              >
                {serviceData?.cta?.buttonText || 'Contact Us'}
              </Button>
            </motion.div>
          </Container>
        </section>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-12 items-center" style={{ direction: isRtl ? 'rtl' : 'ltr' }}>
              ...
              >
                ...
                {serviceData.overview ? (
                  Array.isArray(serviceData.overview) ? (
                    serviceData.overview.map((paragraph, index) => {
                      // Cast to array since we're already inside the Array.isArray check
                      const overviewArray = serviceData.overview as string[];
                      const isLastParagraph = index === overviewArray.length - 1;
                      return (
                        <p key={index} className={isLastParagraph ? "" : "mb-4"}>
                          {paragraph}
                        </p>
                      );
                    })
                  ) : typeof serviceData.overview === 'string' ? (
                    <p>{serviceData.overview}</p>
                  ) : typeof serviceData?.overview === 'object' && serviceData?.overview !== null ? (
                    Object.values(serviceData.overview).map((content, index, array) => {
                      const isLastItem = index === array.length - 1;
                      return (
                        <p key={index} className={isLastItem ? "" : "mb-4"}>
                          {String(content)}
                        </p>
                      );
                    })
          ...
          <Container>
            ...
            >
              <div className="max-w-3xl mx-auto text-center">
                            <h2 className="text-3xl font-bold mb-4">{serviceData?.cta?.title || 'Contact Us'}</h2>
